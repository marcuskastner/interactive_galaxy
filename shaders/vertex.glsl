...
vec3 getOffset(vec3 p){
  ...
  vec3 temppos = rotation3dY(uTime * 0.5 + 0.5 * twist_scale + length(pos.xz)) * p;
  vec3 offset = fbm_vec3(pos, 0.5, 0.0);
  return offset * 0.175;
}
...
void main(){
  ...
  vec3 world_pos = rotation3dY(uTime * 0.3 * (0.1 + 0.5 *  particle_size)) * pos;

  vec3 offset0 = getOffset(world_pos);
  vec3 offset = fbm_vec3(world_pos, 0.0, 0.0);

  vec3 particle_position = (modelMatrix * vec4(world_pos + offset + offset0, 1.0)).xyz;

  float distanceToMouse = clamp(0.2 -length(uMouse.xz - particle_position.xz), 0.0, 1.0);

  vec3 dir = particle_position - uMouse;


  particle_position= mix(particle_position, uMouse + normalize(dir), distanceToMouse);

  vec4 view_pos = viewMatrix * vec4(particle_position, 1.0);
  view_pos.xyz += position * uSize * (0.01 + 0.1* particle_size);
